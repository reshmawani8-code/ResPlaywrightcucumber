...
class AllureCucumberReporter extends _cucumber.Formatter {
  ...
  getHookName(hookId) {
    var _ref4, _this$beforeHooks$hoo, _this$beforeHooks$hoo2, _this$afterHooks$hook;
    return (_ref4 = (_this$beforeHooks$hoo = (_this$beforeHooks$hoo2 = this.beforeHooks[hookId]) === null || _this$beforeHooks$hoo2 === void 0 ? void 0 : _this$beforeHooks$hoo2.name) !== null && _this$beforeHooks$hoo !== void 0 ? _this$beforeHooks$hoo : (_this$afterHooks$hook = this.afterHooks[hookId]) === null || _this$afterHooks$hook === void 0 ? void 0 : _this$afterHooks$hook.name) !== null && _ref4 !== void 0 ? _ref4 : "hook";
  }
  installTestRunHookGlobalErrorSupport(hooks, type) {
    hooks.forEach(hook => {
      var originalCode = hook.code;
      var hookName = type === "before" ? "BeforeAll hook" : "AfterAll hook";
      var runTestRunHook = this.runTestRunHook.bind(this);
      var runCallbackTestRunHook = this.runCallbackTestRunHook.bind(this);
      hook.code = originalCode.length > 0 ? function (callback) {
  ...
  runCallbackTestRunHook(name, originalCode, thisArg, callback) {
    var finished = false;
    var finish = (error, result) => {
      if (!finished) {
        finished = true;
        if (error) {
          this.emitTestRunHookGlobalError(name, error);
    ...
    try {
      var result = originalCode.call(thisArg, finish);
      if (isPromiseLike(result)) {
  ...
  emitTestRunHookGlobalError(name, error) {
    this.allureRuntime.applyGlobalRuntimeMessages([toGlobalErrorMessage(name, (0, _sdk.getMessageAndTraceFromError)(this.toError(error)))]);
  }
  toError(error) {
  ...
  parseTagsLinks(tags) {
    ...
    Object.entries(this.linksConfigs).forEach(_ref5 => {
      var [type, matcher] = _ref5;
      var matchedTags = tags.filter(tag => matcher.pattern.some(pattern => pattern.test(tag.name)));
  ...
  onTestStepFinished(data) {
    ...
    if (step.hookId) {
      ...
      if (error) {
        this.allureRuntime.applyGlobalRuntimeMessages([toGlobalErrorMessage(this.getHookName(step.hookId), (0, _sdk.getMessageAndTraceFromError)(error))]);
      }
...
var deferReject = error => new Promise((_resolve, reject) => setImmediate(() => reject(error)));
